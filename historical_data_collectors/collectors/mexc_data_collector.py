...
# MAX_MEXC_API_LIMIT = 1000
# MEXC_RATE_LIMIT = 1000

# class MexcDataCollector(BaseDataCollector):
...
#         self.fetch_and_write_symbol_trades('ETH/USDT', start_time, end_time)
...
#         query_start_time = int(one_hour_before.timestamp() * 1000)
...
#                 # trades = self.exchange.fetch_trades(symbol, since=query_start_time, limit = MAX_MEXC_API_LIMIT, params = {'until': query_end_time})
#                 trades = self.exchange.fetch_trades(symbol, since=query_start_time, params = {'until': query_end_time})
#                 # trades = self.exchange.fetch_trades(symbol, since=query_start_time, limit = MAX_MEXC_API_LIMIT)
#                 self.profiler.stop('fetch_trades call')

#                 print(self.exchange.iso8601(query_start_time), len(trades), 'trades')
#                 print(self.exchange.iso8601(query_end_time), len(trades), 'trades')
...
#                         l2_trades = super().normalize_to_l2(trades_to_write, 'Mexc')
...
#                         previous_trade_id = first_trade['id']


#                     # only one trade happened in the one hour before end_time. We've already written that trade to database.
#                     # increase start_time by an horu
#                     else:
#                         end_time -= ONE_HOUR_IN_MILLISECONDS + 1
...
#             if trades[len_trades - i - 1]['id'] == previous_trade_id:
#                 # print(len(trades))
#                 # print(len())
#                 return trades[:i]

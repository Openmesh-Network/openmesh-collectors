...
class L3OrderBookManager:
    def __init__(self):
        self.buys = OrderBookTable()
        self.sells = OrderBookTable()
    ...
    def _insert(self, lob_event):
        ...
        order = {"order_id": order_id, "price": price, "size": size}
        order_book.put_dict(order)
        order['side'] = side
    ...
    def _delete(self, lob_event):
        ...
        order_book = self.buys if side == 1 else self.sells
        self._pop_order(order_book, order_id)
        del self.orders[order_id]

        if (side == 1 and order_id == self.best_buy_order["order_id"]) or \
                (side == 2 and order_id == self.best_sell_order["order_id"]):
            self._update_best_levels(side)

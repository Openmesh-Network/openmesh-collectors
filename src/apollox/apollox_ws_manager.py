...
class ApolloXWebsocketManager():
    ...
    def get_snapshot(self):
        ...
        print(self.snapshot)
        self.producer.produce(f"apollox-raw", key="%s:%s" % ("ApolloX", self.url), value=json.dumps(self.snapshot), on_delivery=self._acked)
    ...
    def _on_message(self, ws, message):
        ...
        if isinstance(message, dict):
            ...
            try:
                symbol = self.symbol
                if self.producer:
                    self.producer.produce(f"apollox-raw", key="%s:%s" % ("ApolloX", self.url), value=json.dumps(message), on_delivery=self._acked)
                    self.producer.poll(0)
            except:
                pass
